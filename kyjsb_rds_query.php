...
$stmt = $dbh->prepare("select uid,uname from usertb where userstate_id <> 0");
$stmt->execute();
$rows = $stmt->fetchAll();
                    ...
                    if (start_time == '') {
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                ...
                                trStr += '<tr>';
                                trStr += '<td colspan=10><a href="./kyjsb_rds_print.php?start_time=' + start_time + '&stop_time=' + stop_time + '&userid=' + userid + '" class="btn btn-outline-success" target="_blank">打印</a>';
                                trStr += '</td>';
                                trStr += '</tr>';
                                $("#tbody").html(trStr);
                            }
