    ...
    <main class="flex-shrink-0">
        ...
        <div class="container mt-lg-3">
            <table class="table table-hover text-primary">
                ...
                <tbody>
                <?php
                $sth = $dbh->prepare("select * from work_order where work_state = 0 order by id desc");
                $sth->execute();
                $rows = $sth->fetchAll();
                $length = count($rows);
                for ($i = 0; $i < $length; $i++) { ?>
                    <tr>
                        ...
                        <td>
                            <a href="work_order_update.php?id=<?php echo base64_encode($rows[$i]['id']); ?>"><?php echo $rows[$i]['pro_id']; ?></a>
                        </td>
                        <td><?php echo $rows[$i]['bath_number']; ?></td>
                        <td>
                            <?php if ($rows[$i]['technology_target'] == 'YM') {
                                echo '研磨';
                            } else {
                                echo '分散';
                            }
