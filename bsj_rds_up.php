    ...
    <main class="flex-shrink-0">
        <div class="container mt-lg-auto">
            <table class="table caption-top table-hover table-success text-primary table-sm text-center">
                <caption><h4><span style="color: black">未关停冰水设备</span></h4></caption>
                <thead>
                <tr>
                    <th scope="col">#</th>
                ...
                <tbody id="tbody">
                <?php
                $stmt = $dbh->prepare("SELECT id,machine_id,register_date,register_time,machine_status,uname FROM bsjjl_show where machine_status='开机' order by register_date,register_time");
                $stmt->execute();
                $rows = $stmt->fetchAll();
                $length = count($rows);
                for ($i = 0; $i < $length; $i++) { ?>
