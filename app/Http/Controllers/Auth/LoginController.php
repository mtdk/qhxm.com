...
class LoginController extends Controller
{
    ...
    public function checkRememberedUser(Request $request)
    {
        ...
        // 调试日志（实际部署时应移除）
        \Log::debug('checkRememberedUser called', [
            'requested_uid' => $uid,
            'remembered_uid' => $rememberedUid,
            'has_password' => !empty($rememberedPassword),
            'password_length' => strlen($rememberedPassword ?? ''),
        ]);

        if ($uid === $rememberedUid && $rememberedPassword) {
            // 尝试解密cookie（如果被加密）
            try {
                $decryptedPassword = $rememberedPassword;
                // 如果cookie被加密，尝试解密
                if (config('session.encrypt') && function_exists('decrypt')) {
                    $decryptedPassword = decrypt($rememberedPassword, false);
                }
            } catch (\Exception $e) {
                $decryptedPassword = $rememberedPassword;
            }
