    ...
    <main class="flex-shrink-0">
        <div class="container mt-lg-auto">
            <table class="table caption-top table-hover table-success text-primary table-sm text-center">
                ...
                <tbody id="tbody">
                <?php
                $stmt = $dbh->prepare("SELECT id,machine_id,register_date,register_time,machine_state,uname FROM bsjjl_show where machine_state='开机' order by register_date,register_time");
                $stmt->execute();
                $rows = $stmt->fetchAll();
                $length = count($rows);
                for ($i = 0; $i < $length; $i++) { ?>
