...
$sql = "SELECT machine_id,register_date,register_time,shutdown_time,total_duration,uname FROM fqpfsbjl_show where register_date>=? and register_date<=? and uid=? order by register_date,register_time";
...
$pdf->Cell(35, 5, '启动日期', 1, 0, 'C');
$pdf->Cell(35, 5, '启动时间', 1, 0, 'C');
$pdf->Cell(35, 5, '关闭时间', 1, 0, 'C');
$pdf->Cell(30, 5, '总时长(分钟)', 1, 0, 'C');
...
//    $pdf->Write(0, $row[1], '', 0, 'L', true, 0, false, false, 0);
  ...
  $pdf->Cell(35, 5, $row[1], 1, 0, 'C');
  $pdf->Cell(35, 5, substr($row[2], 0, 5), 1, 0, 'C');
  $pdf->Cell(35, 5, substr($row[3], 0, 5), 1, 0, 'C');
