        ...
        <div class="accordion accordion-flush" id="accordionFlushExample">
            ...
            <div class="accordion-item">
                <h2 class="accordion-header" id="flush-headingThree">
                    ...
                    // $rd 当前系统时间
                    $rd = date('Y-m-d');
                    $stmt = $dbh->prepare("SELECT id,machine_id,register_time,machine_status from fqssrecords where register_date=:register_date and machine_status='开机'");
                    $stmt->bindParam(':register_date', $rd);
                    $stmt->execute();
                    $rows = $stmt->fetchAll();
                    $length = count($rows);
            ...
            <div class="accordion-item">
                <h2 class="accordion-header" id="flush-headingFour">
                    <?php
                    $rd = date('Y-m-d');
                    $stmt = $dbh->prepare("SELECT id,machine_id,register_time,machine_status from kyjrecords where register_date=:register_date and machine_status='开机'");
                    $stmt->bindParam(':register_date', $rd);
                    $stmt->execute();
                    $rows = $stmt->fetchAll();
                    $length = count($rows);
            ...
            <div class="accordion-item">
                <h2 class="accordion-header" id="flush-headingFive">
                    <?php
                    $rd = date('Y-m-d');
                    $stmt = $dbh->prepare("SELECT id,machine_id,register_time,machine_status from bsjrecords where register_date=:register_date and machine_status='开机'");
                    $stmt->bindParam(':register_date', $rd);
                    $stmt->execute();
                    $rows = $stmt->fetchAll();
                    $length = count($rows);
