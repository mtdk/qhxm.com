...
if (empty($id)) {
  ...
  $stmt = $dbh->prepare("SELECT id,machine_state from bsjrecords where id=:id and machine_state='关机'");
  ...
  $sql = "update bsjrecords set shutdown_time=:shutdown_time,total_duration=:total_duration,machine_state=:machine_status,machine_status=:radioMachineStatus where id=:id";
