...
if (empty($machine_id)) {
    ...
    $sql = "SELECT machine_id FROM bsjrecords WHERE machine_id=:machine_id and machine_state=:machine_status";
    ...
    if ($radioMachineStatus == "正常") {
        $sql = "INSERT INTO bsjrecords (machine_id,register_date,register_time,machine_state,machine_status,uid) VALUES (?,?,?,?,?,?)";
        $stmt = $dbh->prepare($sql);
        ...
        $machine_status = "关机";
        $sql = "INSERT INTO bsjrecords (machine_id,register_date,register_time,shutdown_time,total_duration,machine_state,machine_status,uid) VALUES (?,?,?,?,?,?,?,?)";
        $stmt = $dbh->prepare($sql);
        $stmt->bindValue(1, $machine_id);
        $stmt->bindValue(2, $register_date);
        $stmt->bindValue(3, $register_time);
