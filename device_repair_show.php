...
?>
    <main class="flex-shrink-0">
        <table class="table table-hover text-primary">
            ...
            <tbody>
            <?php
            $stmt = $dbh->prepare("select repair_id,device_name,use_name,apply_time from tb_device_repair_order where repair_status = 0");
            $stmt->execute();
            $rows = $stmt->fetchAll();
            $length = count($rows);
            for ($i = 0; $i < $length; $i++) { ?>
                <tr>
                    ...
                    <td><a href="device_repair_check.php?repair_id=<?php echo base64_encode($rows[$i]['repair_id']); ?>"
                           class="btn btn-outline-success btn-sm">查看</a>
                    </td>
                </tr>
            <?php } ?>
            </tbody>
